...
class ProductVersion(AbstractModel):
    ...
    def main_image(self):
        return self.productimage.all().filter(is_main=True).first()
    ...
    def other_image(self):
        return  self.productimage.exclude(id=self.productimage.all().filter(is_main=True).first().id)
...
class WishList(AbstractModel):
    productversion=models.ManyToManyField(ProductVersion,related_name='wishlist',blank=True)
